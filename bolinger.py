...
while True:
      ...
      def bolingerband(symbol,width,intervalunit,length):
            if intervalunit=='1T':
                start_st='100 minutes ago UTC'
                interval_data='1m'

                D=pd.DataFrame(
                    client.get_historical_klines(symbol=symbol, interval=interval_data, start_str=start_st)
                )
                ...
                D['symbol'] =symbol
                D=D[['symbol','open_date_time','open','high','low','close','volume','close_time','qav','num_trades','taker_base_vol','taker_quote_vol']]
      ...
      bb_1m=bolingerband((symbol_trade), width, '1T', length)
